...
+ (BOOL)isiPhone5or4 {
    return (MIN(UIScreen.mainScreen.bounds.size.width, UIScreen.mainScreen.bounds.size.height) == 320);
}
+ (CGFloat)screenWidth {
...
+(UIFont *)PingFangSCRegularAndSize:(CGFloat)size{
    ...
    return [UIFont fontWithName:@"PingFangSC-Regular" size:DDPlayerTool.isiPhone5or4 ? size-1 : size];
}

+(UIFont *)pingfangSCSemiboldAndSize:(CGFloat)size{
    return [UIFont fontWithName:@"PingFangSC-Semibold" size:DDPlayerTool.isiPhone5or4 ? size-1 : size];
}
