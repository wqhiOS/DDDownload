...
- (void)captureVideoWithAsset:(AVAsset *)asset startTime:(CMTime)startTime duration:(CGFloat)duration success:(nonnull void (^)(NSString*))success failure:(nonnull void (^)(NSError * _Nonnull))failure {
    ...
    //截取当前视频的帧画面 并展示
    [self showCurrntVideoImage:[DDPlayerManager thumbnailImageWithAsset:asset currentTime:CMTimeMakeWithSeconds(CMTimeGetSeconds(startTime) + duration, NSEC_PER_SEC)] completion:^{
        //截取视频
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if (weakSelf.isClickBackButton) {
                [weakSelf.assetExportSession cancelExport];
            }
        });
    }];
...
- (void)showCurrntVideoImage:(UIImage *)currentVideoImage completion:(void(^)(void))completion{
...
- (void)captureProgress:(CGFloat)progress {
    ...
    NSLog(@"截取进度更新：progress: %lf",progress);
...
- (void)initialize {
    ...
    [self.playerBgView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self).mas_offset(80);
        make.width.mas_equalTo(DDPlayerTool.screenHeight * 0.3);
        make.height.mas_equalTo(DDPlayerTool.screenHeight * 0.3 * 9/16);
        make.centerX.equalTo(self);
    }];
...
- (UILabel *)commentLabel {
    if (!_commentLabel) {
        ...
        _commentLabel.font = [DDPlayerTool PingFangSCRegularAndSize:16];
