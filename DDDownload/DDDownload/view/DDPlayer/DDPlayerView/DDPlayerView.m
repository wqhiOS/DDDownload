...
- (void)playerReadyToPlay {
    if (_WWANWarnView && _WWANWarnView.superview) {
        [_WWANWarnView removeFromSuperview];
    }
    if (_networkErrorView && _networkErrorView.superview) {
        [_networkErrorView removeFromSuperview];
    }
    if (_playerErrorView && _playerErrorView.superview) {
        [_playerErrorView removeFromSuperview];
    }
    ...
    //重置速率
    [self.playerControlView.bottomLandscapeView.rateButton setTitle:[NSString stringWithFormat:@"%.1lfX",MAX(self.player.rate, 1.0)] forState:UIControlStateNormal];
...
- (void)playerNetworkStatusChanged:(NetworkStatus)networkStatus {
    switch (networkStatus) {
        ...
        case ReachableViaWiFi:
        {
            ...
            if (self.player.currentAsset.URL.absoluteString && self.player.currentAsset.URL.absoluteString.length > 0) {
                [self.player play];
            }else {
                [self.player playWithUrl:[self.player valueForKey:@"_willPlayUrlString"]];
            }
        }
            break;
        default:
...
- (void)playerControlViewDragingProgress:(CGFloat)value {
    ...
    if (dragProgressView.superview == nil) {
        [self addSubview:dragProgressView];
        [dragProgressView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(self);
        }];
    }
    if (DDPlayerTool.isScreenPortrait) {
...
-(void)playerControlViewEndDragProgress:(CGFloat)value {
    ...
    CGFloat lastRate = self.player.rate;
    __weak typeof(self) weakSelf = self;
    [self.player seekToTime:self.player.duration * value isPlayImmediately:self.player.isPlaying completionHandler:^(BOOL isComlete) {
        [weakSelf.player playImmediatelyAtRate:lastRate];
    }];
