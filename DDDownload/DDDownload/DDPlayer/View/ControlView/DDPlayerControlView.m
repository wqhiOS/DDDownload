...
- (void)updateUIWithLandscape {
    ...
    if (self.superview) {
        DDPlayerView *playerView = (DDPlayerView*)self.superview;
        self.captureVideoButton.hidden = playerView.isHiddenCapture;
        self.captureImageButton.hidden = playerView.isHiddenCapture;
    }
}
...
- (void)dismiss {
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        NSLog(@"%s finished:%d",__FUNCTION__,finished);
    }];
